    ...
    <form v-on:submit.prevent="updateComment">
      <div class="row">
        ...
        <div class="col-4 d-flex align-items-center">
          <button type="submit" @click="updateComment(post)" class="btn btn-pink">수정완료</button>
        </div>
      </div>
    </form>
  </div>
...
export default {
  ...
  methods: {
    ...
    updateComment: function () {
      ...
      axios
        .put(`${SERVER_URL}/community/${this.post.id}/comment/${this.updateCommentItem.id}/`, updateItem, config)
        .then((res) => {
          console.log(res);
          this.$emit('trigger');
